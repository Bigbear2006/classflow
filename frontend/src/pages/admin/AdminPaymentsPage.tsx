...
import { DollarSign, X, Check, ChevronDown, ChevronUp, Users } from 'lucide-react';
import type {
        ...
        <div className="space-y-4">
          {groups.map(group => {
            return (
              ...
              >
                <button
                  onClick={() => setExpandedGroups(groups => [...groups, group.id])}
                    ...
                    {group.course.paymentType === 'FULL_COURSE' ? (
                      ...
                      <div className="divide-y divide-slate-50">
                        {group.students.map(student => {
                          const pct = Math.min(
                            Math.round((student.totalPaid / group.course.price) * 100),
                            100,
                          );
                          return (
                              ...
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="text-right">
                                    ...
                                    {student.totalPaid >= group.course.price && (
                                      <span className="ml-1 text-xs text-emerald-600">✓</span>
                                    )}
                                  </div>
                                  <button
                                    onClick={() => openGroupPay(group.id, student.id)}
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 ml-9">
                                  {group.lessons.map(lesson => {
                                    ...
                                    return (
                                      <button
                                        key={lesson.id}
                                        onClick={() => !paid && openIndPay(lesson.id, student.id)}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {student.lessons.map(lesson => {
                      ...
                      return (
                        <button
                          key={lesson.id}
                          onClick={() => !paid && openIndPay(lesson.id, student.student.id)}
